...
!JaV
    character(LEN=20) :: ede_file
    real(dl)  :: omegab, omegac, omegav, omegan, omegaede
...
500     if (L< lmax_extrap_highl) &
    ...
    do ik=1,MTrans%num_q_trans
        ...
        do itf = 1, nz
            ...
            *pi*twopi*h**3*power)
!print *,'1111', k, PK_data%matpower(ik,itf)
        end do
        !JaV 
        !print *, k, PK_data%matpower(ik,itf)
    end do
!print *,'1111', k, PK_data%matpower(ik,itf)
    ...
    do i_PK=1, CP%Transfer%PK_num_redshifts
        ...
        !JaV
       ! print *,'666', MTrans%TransferData(Transfer_kh:Transfer_max,ik,i)
                end if
            end do
            close(fileio_unit)
        end if
        ...
        if (FileNames(itf) /= '') then
            if (.not. transfer_interp_matterpower ) then
                ...
                do i=1,points
                    ...
                    !JaV
                    !print *, minkh*exp((i-1)*dlnkh),outpower(i,1:CP%InitPower%nn,1)
                end do
                close(fileio_unit)
            end if
...
!JaV
!print *, CP%ede_file
open (51,file=CP%ede_file)
NN = 100
dda = (1. - 1d-6)/NN
do i=0, NN
  aa = 1d-6 + i*dda
write (51,'(3G)'), 1.0/aa-1.0, AngularDiameterDistance(1./aa - 1), 1./(HofZ(1./aa -1))
end do
close(51)
...
!print *, 'z=2.36', (c*Hofz(2.36d0)/1.d3), rs,  AngularDiameterDistance(2.36d0)
...
!print *, 'z=2.34', (c*Hofz(2.34d0)/1.d3), rs,  AngularDiameterDistance(2.34d0)
