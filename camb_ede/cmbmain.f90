    ...
    if (fixq/=0._dl) then
        ...
        do j=1,1000
            ...
            !ddelta= (yprime(3)*grhoc+yprime(4)*grhob+yprime(EV%w_ix)*grho_de(y(1)))/(grhob+grhoc+grho_de(y(1)))
            delta=(grhoc*y(3)+grhob*y(4))/(grhob+grhoc)
            growth= ddelta/delta/adotoa
            write (1,'(7E15.5)') tau, delta, growth, y(3), y(4), y(EV%g_ix), y(1)
!JAV
!            print *, y(1), growth,adotoa !'something'    
            !print *, y(1), adotoa !y(3), y(EV%w_ix) !EV%w_ix, grho_de(y(1))*y(EV%w_ix), yprime(EV%w_ix)*grho_de(y(1))   
        end do
        close(1)
        stop
    end if
